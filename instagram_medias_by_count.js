...
nick.newTab().then(async (tab) => {
  ...
  const medias = await tab.evaluate((arg, callback) => {
    ...
    // utility function to extract media nodes from the user edge
    const extractMediaEdges = edgeWithMedias => _.get(edgeWithMedias, 'edge_owner_to_timeline_media.edges', []);

    // media edges will be pushed to this array until we're out of results or reach our desired count
    const mediaEdges = [];

    // We already have up to 12 media results, so let's use them!
    _.forEach(extractMediaEdges(userEdge), function(mediaEdge) {
      if (mediaEdges.length >= count) return false;
      mediaEdges.push(mediaEdge);
    });

    const generateGisHeader = (variables) => md5(`${rhxGis}:${csrfToken}:${variables}`);
    ...
    if (count - mediaEdges.length > 0) {
      ...
      while (hasNextPage && count - mediaEdges.length > 0) {
        const nextPage = fetchNextPage(endCursor);
        const nextPageMediaEdges = extractMediaEdges(nextPage);

        _.forEach(nextPageMediaEdges, function(mediaEdge) {
          if (mediaEdges.length >= count) return false;
          mediaEdges.push(mediaEdge);
        });
    ...
    // build the final return objects
